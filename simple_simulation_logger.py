...
class GR1SimulationLogger:
    def __init__(self, urdf_path=URDF_PATH):
        ...
        self.robot = self.scene.add_entity(gs.morphs.URDF(file=urdf_path, pos=(-0.2, 0, 0.95), fixed=True))
        self.table = self.scene.add_entity(gs.morphs.Box(pos=(0.45, 0, 0.4), size=(0.4, 0.5, 0.8), fixed=True))
        self.cube = self.scene.add_entity(gs.morphs.Box(pos=(0.45, -0.2, 0.82), size=(0.04, 0.04, 0.04), fixed=False),
                                          surface=gs.surfaces.Default(color=(1, 0, 0)))

        # Cameras (224x224 typical for foundation models)
        self.world_cam_top = self.scene.add_camera(res=(224, 224), fov=60, pos=(0.3, 0, 1.8), lookat=(0.3, 0, 0.8))
        self.world_cam_left = self.scene.add_camera(res=(224, 224), fov=60, pos=(1.0, -1.0, 1.2), lookat=(0, 0, 0.8))
        self.world_cam_right = self.scene.add_camera(res=(224, 224), fov=60, pos=(1.0, 1.0, 1.2), lookat=(0, 0, 0.8))
        self.world_cam_center = self.scene.add_camera(res=(224, 224), fov=60, pos=(1.0, 0.0, 1.2), lookat=(0, 0, 0.8))
    ...
    def get_state_32(self):
        ...
        normalized_state = (raw_state - JOINT_LIMITS_MIN) / (JOINT_LIMITS_MAX - JOINT_LIMITS_MIN + 1e-8) * 2.0 - 1.0
    ...
    def run(self, sub_port=5556, pub_port=5557):
        ...
        print(f"Listening for Actions on {sub_port} | Publishing Observations on {pub_port}")
        ...
        while self.is_running:
            ...
            for i in range(100):
                ...
                if i % 10 == 0:
                    ...
                    rr.log("world_center", rr.Image(rgb_center[..., :3].astype(np.uint8)))
