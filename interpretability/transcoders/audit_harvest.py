...
if str(ROOT_DIR / "lewm") not in sys.path: sys.path.append(str(ROOT_DIR / "lewm"))
...
def audit_activations(output_dir, model_path):
    ...
    for name, module in model.named_modules():
        parts = name.split(".")
        if len(parts) > 1 and (parts[-2] == "layer" or parts[-2] == "layers") and parts[-1].isdigit():
    ...
    for layer_id in tqdm(found_layer_ids, desc="Auditing Layers"):
        ...
        if expected_bytes != actual_bytes:
            report.append({"layer": layer_id, "status": f"❌ Byte Mismatch (Expected {expected_bytes}, Found {actual_bytes})"})
            continue
        ...
        data = np.memmap(bin_file, dtype=np.float16, mode="r", shape=tuple(meta["shape"]))
        ...
        if nan_found:
             report.append({"layer": layer_id, "status": "⚠️ Value Fidelity Failure (NaN/Inf detected)"})
        else:
             report.append({"layer": layer_id, "status": "✅ Healthy", "samples": meta["shape"][0], "dims": meta["shape"][1]})

    # 3. Cross-Layer Alignment
    print("\n🔗 Step 3: Cross-Layer Alignment...")
    if len(set(total_samples_list)) > 1:
        print(f"  ❌ ALIGNMENT FAILURE: Layers have different sample counts! {set(total_samples_list)}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=str, required=True, help="Path to harvested activations")
