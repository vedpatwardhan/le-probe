...
class GR1Simulation:
    def __init__(self, urdf_path=URDF_PATH, test_mode=False):
        ...
        # Save the natural urdf home pose and the ee link for IK
        self.home_q = self.robot.get_qpos().clone()
        self.ee_link = self.robot.get_link("right_hand_roll_link")

        # Aggressive control profile
        self.robot.set_dofs_kp(np.full(self.robot.n_dofs, 80000.0))
        self.robot.set_dofs_kv(np.full(self.robot.n_dofs, 1200.0))
        self.robot.set_dofs_force_range(
            np.full(self.robot.n_dofs, -5000.0),
            np.full(self.robot.n_dofs, 5000.0),
        )
        ...
        # Pre-compute protocol limit and mapping tensors
        ...
        # Configure finger coupling
        self.configure_finger_coupling()
        ...
        print(
            f"✅ Unified Mask initialized with {len(self.allowed_32_indices)} joints."
        )

        # Also maintain raw DOF indices for IK solver and calculate base offset
        self.ik_dof_indices = []
        ...
        self.joint_dof_map = self.get_joint_dof_map()
        for i in sorted(self.allowed_32_indices):
            mapping = self.joint_dof_map[i]
            print(
                f"  - Authorized Proxy [{i:02}] -> Robot DOF "
                f"{mapping['dof_idx']} ({mapping['name']})"
            )

    def configure_finger_coupling(self):
                ...
                for other_joint in self.robot.joints:
                    if other_joint.name and finger_prefix in other_joint.name:
                        if (
                            ...
                            coupling_sources.append(dof_idx)
    ...
    def get_joint_dof_map(self):
        """Get diagnostics used for tests."""
        legacy_map = []
    ...
    def solve_ik(self, pos, quat):
        """Constrains the Jacobian to native authorized joints."""
        ...
        # Overwrite specifically with the new IK results where intended
        q_final[self.ik_dof_indices] = q_proposed_clamped

        return q_final

    def get_state_32(self):
        """Returns the current 32-dof joint positions as a numpy array."""
    ...
    def _normalize_state(self, raw_state):
        ...
        range_val = torch.where(range_val > 1e-4, range_val, 1e-4)
    ...
    def reset_env(self):
        ...
        # Force Waist Pitch to be forward/ready
    ...
    def process_target_32(self, action_32):
        ...
        # Parallel finger coupling (mimic proximal -> distal)
        if len(self.v_coupling_targets) > 0:
