...
def test_pretrained_weight_ranks():
    ...
    if not os.path.exists(CKPT_PATH):
        pytest.skip(f"Pre-trained weights not found at {CKPT_PATH}. Skipping benchmark.")
    ...
    for key, weight in state_dict.items():
        if "weight" in key and len(weight.shape) == 2:
            ...
            assert diagnostics["soft_rank"] > 5.0, f"Layer {key} has abnormally low rank: {diagnostics['soft_rank']}"
    ...
    if not os.path.exists(CKPT_PATH):
        ...
        for k, w in sd.items():
            if "weight" in k and len(w.shape) == 2 and w.shape[0] >= 16:
                ...
                print(f"{k[:60]:<60} | {d['soft_rank']:<10.2f} | {d['participation_ratio']:<10.2f}")
