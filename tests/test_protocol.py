...
def test_32_dof_mapping_consistency(sim):
    ...
    for target_idx in range(32):
        ...
        if target_idx in sim.allowed_32_indices:
            ...
            assert abs(actual_rad - expected_rad) < 1e-4, f"Mapping error at index {target_idx} ({joint_name})!"
        else:
            # Verify it was rejected
            actual_rad = sim.last_target_q[ground_truth_dof].item()
            home_rad = sim.home_q[ground_truth_dof].item()
            assert abs(actual_rad - home_rad) < 1e-6, f"Unauthorized joint {joint_name} bypassed the gain at index {target_idx}!"
...
def test_finger_coupling_logic(sim):
    ...
    assert len(mapping["coupled"]) > 0, f"Joint {mapping['name']} should have coupled DOFs!"
    ...
    for coupled_dof_idx in mapping["coupled"]:
        ...
        assert abs(coupled_val - target_val) < 1e-6, f"Coupled DOF {coupled_dof_idx} failed to sync with proximal joint!"
...
def test_unauthorized_joint_rejection(sim):
    ...
    assert final_val == original_val, "Unauthorized joint was able to bypass the intake gate!"
