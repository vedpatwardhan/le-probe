...
def test_finger_coupling_propagation(sim):
    ...
    assert len(mapping["coupled"]) > 0, f"Index {prox_idx} ({mapping['name']}) has no coupled joints!"
    ...
    for c_idx in mapping["coupled"]:
        ...
        assert abs(actual_rad) > 1e-4, f"Coupled DOF {c_idx} failed to update! Actual: {actual_rad}"
...
def test_adversarial_input_robustness(sim):
    ...
    assert abs(sim.last_target_q[mapping["dof_idx"]] - limit_max) < 1e-4, "Failed to clip excessive input!"
    
    # 2. NaN in an authorized joint (should be ignored)
    prev_val = sim.last_target_q[mapping["dof_idx"]]
    action_32[16] = np.nan
    sim.process_target_32(action_32)
    assert sim.last_target_q[mapping["dof_idx"]] == prev_val, "NaN input erroneously modified target!"
