...
def test_mps_to_numpy_safe_extraction(sim):
    ...
    target_q_32 = torch.stack(
        [target_q[m["dof_idx"]] for m in sim.joint_dof_map]
    )
...
def test_waist_randomization_boundaries(sim):
    ...
    for _ in range(10):
        ...
        normalized_pitch = (actual_rad - limit_min) / (limit_max - limit_min) * 2.0 - 1.0
        
        # We allow a tiny tolerance for floating point epsilon
        assert -0.61 <= normalized_pitch <= -0.16, f"Waist pitch {normalized_pitch:.3f} fell outside authorized ready-pose range!"
